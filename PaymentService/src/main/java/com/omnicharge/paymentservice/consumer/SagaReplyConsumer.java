...
public class SagaReplyConsumer {
    ...
    /**
     * A message landed in the dead-letter queue, meaning RechargeProcessing
     * failed to process the saga event even after all RabbitMQ retries.
     *
     * Compensation steps:
     *  1. Mark the Transaction as FAILED (it was SUCCESS after Razorpay verified)
     *  2. Issue a real Razorpay refund via RazorpayRefundService
     *  3. Send a refund email notification to the user
     */
        ...
        try {
            ...
            transactionRepository.findById(transactionId).ifPresentOrElse(txn -> {
                ...
                if (txn.getStatus() == TransactionStatus.SUCCESS) {
                    ...
                    // ── NEW: Issue real Razorpay refund + send refund email ──────
